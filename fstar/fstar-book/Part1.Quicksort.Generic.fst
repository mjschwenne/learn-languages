...
let sort_ok (#a:eqtype) (f:total_order_t a) (l:list a) = 
  let m = sort f l in 
    sorted f m /\ (forall i. mem i l = mem i m)
...
let rec sort_correct (#a:eqtype) (f:total_order_t a) (l:list a)
  ...
  = match l with 
    ...
    | pivot :: tl -> let hi, lo = partition (f pivot) tl in 
                   ...
                   // Added assertions to track down typo in partition_mem 
                   // post-condition. '=' was '==>'
                   assert (forall i. mem i tl = (mem i hi || mem i lo));
                   assert (forall i. mem i hi ==> f pivot i);
                   assert (forall i. mem i lo ==> f i pivot);
                   assert (sort f l == (append (sort f lo) (pivot :: sort f hi)));
                   sorted_concat f (sort f lo) (sort f hi) pivot;
                   assert (sorted f (sort f l));
                   append_mem (sort f lo) (pivot :: sort f hi);
                   assert (forall i. mem i l = mem i (sort f l))
