...
let rec partition_mem_permutation (#a:eqtype)
  ...
  = match l with 
    | [] -> ()
    | pivot :: tl -> partition_mem_permutation f tl;
                // Delayed due to typo: 'tl' was 'l'
                let hi, lo = partition f tl in
                append_count hi lo;
                append_count hi (pivot :: lo);
                // This last one seems to be having some consistency issues...
                append_count (pivot :: hi) lo
...
let permutation_app_lemma (#a:eqtype) (hd:a) (tl:list a)
  ...
  = append_count l1 l2; append_count l1 (hd :: l2)
...
let rec sort_intrinsic (#a:eqtype) (f:total_order_t a) (l:list a)
  ...
  = match l with 
    ...
    | pivot :: tl -> let hi, lo = partition (f pivot) tl in 
                   ...
                   permutation_app_lemma pivot tl (sort_intrinsic f lo)          
                                                  (sort_intrinsic f hi);
