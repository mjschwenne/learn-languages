...
let rec add_every_number_up_to x =
  match x with 0 -> 0 | _ -> plus x (add_every_number_up_to (x - 1))

let rec factorial x = match x with 0 -> 1 | _ -> times x (factorial (x - 1))
...
let rec sum xs = match xs with [] -> 0 | x :: ys -> plus x (sum ys)
let rec product xs = match xs with [] -> 1 | x :: ys -> times x (product ys)
...
let rec every answer combine xs =
  match xs with [] -> answer | x :: ys -> combine x (every answer combine ys)
