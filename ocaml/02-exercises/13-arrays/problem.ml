...
let double array : unit = Array.iteri array ~f:(fun i x -> array.(i) <- 2 * x)
...
let double_selectively array indices : unit =
  Array.iteri array ~f:(fun i x ->
      if List.mem indices i ~equal:Int.( = ) then array.(i) <- 2 * x else ())
...
let double_matrix matrix : unit = Array.iter matrix ~f:(fun a -> double a)
