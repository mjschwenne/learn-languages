...
(* OCaml variants are in many ways more powerful than Enums because the
   ...
   example: *)
type card_value = Ace | King | Queen | Jack | Number of int
...
let card_value_to_score card_value =
  match card_value with Ace -> 11 | King | Queen | Jack -> 10 | Number i -> i

(* Remember the list type? We can define a list as a variant type too! *)
type int_list = Empty | Not_empty of int * int_list
...
type 'a generic_list = Empty | Not_empty of 'a * 'a generic_list
