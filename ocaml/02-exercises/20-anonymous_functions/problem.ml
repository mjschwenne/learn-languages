...
let map_option f opt = match opt with None -> None | Some i -> Some (f i)
let double i = 2 * i
let () = assert ([%compare.equal: int option] (map_option double None) None)
...
let () =
  assert (
    [%compare.equal: int option] (map_option (fun i -> 2 * i) (Some 2)) (Some 4))
...
let apply_if_nonzero f i = if i = 0 then 0 else f i
...
let add1_if_nonzero i = apply_if_nonzero (fun i -> i + 1) i
let%test "Testing add1_if_nonzero..." = Int.( = ) 0 (add1_if_nonzero 0)
let%test "Testing add1_if_nonzero..." = Int.( = ) 2 (add1_if_nonzero 1)
let%test "Testing add1_if_nonzero..." = Int.( = ) 6 (add1_if_nonzero 5)
